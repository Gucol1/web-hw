...
`npm run preview` - для просмотра собранного проекта
...
`Используйте знания предыдущей части, чтобы подключить инфраструктуру (ts, линтер, сборщик). МОЖНО использовать готовые фреймворки и бойлерплейты с явным указанием и обоснованием выбора того или иного варианта`

**Ответ:** все конфиги остались те же, так как в форматере абсолютно ничего не поменяется, в линтере уже был добавлен плагин для реакта,
в tsconfig был добавлен параметр для react `"jsx": "react-jsx"`, также немного изменился конфиг _Vite_ также подключением плагина для реакта

`Выберите стейт-менеджер, либо подробно обоснуйте почему он не нужен (да, за это баллы тоже можно получить)`

**Ответ:** Использование стейт-менеджера в данном приложении излишне, так как встроенные 
хуки useState полностью покрывают необходимые потребности в управлении состоянием, добавив стейт-менеджера, пришлось бы дольше разрабатывать и собирать,
, ведь стейт-менеджеры оправданы только в крупных проектах со сложным состоянием, частыми обновлениями и необходимостью централизованного управления данными.
